        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('populasi_ayams.store') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2" for="tanggal">Tanggal</label>
                            <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-amber-500" required>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="jenis" id="jenis" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-amber-500" required onchange="toggleHarga(this.value)">
                                <option value="masuk" {{ old('jenis') == 'masuk' ? 'selected' : '' }}>Ayam Masuk (Bertambah)</option>
                                <option value="mati" {{ old('jenis') == 'mati' ? 'selected' : '' }}>Ayam Mati (Berkurang)</option>
                                <option value="terjual" {{ old('jenis') == 'terjual' ? 'selected' : '' }}>Ayam Terjual / Afkir (Berkurang & Pendapatan)</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah') }}" min="1" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-amber-500" required>
                            @error('jumlah') <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p> @enderror
                        </div>
                        
                        <div class="mb-4" id="harga_container" style="display: {{ old('jenis') == 'terjual' ? 'block' : 'none' }};">
                            <label class="block text-gray-700 text-sm font-bold mb-2" for="harga_satuan">Harga per Ekor (Rp) - Khusus Ayam Terjual</label>
                            <input type="number" name="harga_satuan" id="harga_satuan" value="{{ old('harga_satuan') }}" min="0" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-amber-500">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="keterangan" id="keterangan" rows="3" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-amber-500">{{ old('keterangan') }}</textarea>
