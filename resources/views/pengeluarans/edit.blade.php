<x-app-layout>
    ...
    @php
        $kategoriNames = $kategoris->pluck('nama')->toArray();
        $isSelectedInKategori = in_array($pengeluaran->nama_pengeluaran, $kategoriNames);
        $initialSelect = old('nama_pengeluaran_select', $isSelectedInKategori ? $pengeluaran->nama_pengeluaran : 'Lainnya');
        $initialManual = old('nama_pengeluaran_manual', !$isSelectedInKategori ? $pengeluaran->nama_pengeluaran : '');
    @endphp

    <div class="py-12" x-data="{
        selectedOpsi: '{{ $initialSelect }}',
        manualInput: '{{ $initialManual }}',
        nominalInput: '{{ old('nominal', $pengeluaran->nominal) }}',
        modalOpen: false,
        newOpsiNama: '',
        loadingOpsi: false,
        errorMessage: '',
                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('pengeluarans.update', $pengeluaran->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $pengeluaran->tanggal) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-amber-500 focus:ring-amber-500" required>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="keterangan" id="keterangan" rows="3" placeholder="Catatan tambahan..." class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-amber-500 focus:ring-amber-500">{{ old('keterangan', $pengeluaran->keterangan) }}</textarea>
            ...
            <div class="bg-white rounded-xl shadow-2xl transform transition-all sm:max-w-lg sm:w-full z-10 overflow-hidden" @click.stop>
                ...
                <div class="p-6">
                    ...
                    <form @submit.prevent="addOpsi()">
                        <div class="mb-4">
                            <label class="block text-gray-700 text-xs font-bold uppercase mb-1" for="modal_nama_opsi_edit">
                                Nama Pengeluaran Baru
                            </label>
                            <input type="text" id="modal_nama_opsi_edit" x-model="newOpsiNama" placeholder="Contoh: BBM Kendaraan, Konsumsi, dll..." class="shadow-sm appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500" :disabled="loadingOpsi">
                        </div>
